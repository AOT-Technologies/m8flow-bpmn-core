...
print_usage() {
  cat <<'EOF'
Usage:
  ./examples/scheduled_timer_poc.sh [--database-url URL] [--docker] [--postgres-image IMAGE] [--keep-container]
...
main() {
  ...
  m8f_run_python_example "$SCRIPT_DIR" "scheduled_timer_poc.py" "scheduled timer POC"
}
