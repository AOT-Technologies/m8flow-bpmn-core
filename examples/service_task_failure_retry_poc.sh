...
print_usage() {
  cat <<'EOF'
Usage:
  ./examples/service_task_failure_retry_poc.sh [--database-url URL] [--docker] [--postgres-image IMAGE] [--keep-container]
...
main() {
  ...
  m8f_run_python_example "$SCRIPT_DIR" "service_task_failure_retry_poc.py" "service-task failure retry POC"
}
